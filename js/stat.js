...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    var blue = randomInteger(1, 255);

    ctx.fillStyle = names[i] === 'Вы' ? 'rgba(255,0,0,1)' : 'rgba(0, 0, ' + blue + ', 1)';

    var barHeight = (HEIGHT * times[i]) / maxTime;
    ctx.fillText(names[i], CLOUD_X + GAP + (TEXT_WIDTH + GAP) * i, CLOUD_Y + CLOUD_HEIGHT - GAP / 3);
    ctx.fillRect(CLOUD_X + GAP + (BAR_WIDTH + GAP) * i, CLOUD_Y + CLOUD_HEIGHT - barHeight - GAP / 3 - TEXT_HEIGHT, BAR_WIDTH, barHeight);
